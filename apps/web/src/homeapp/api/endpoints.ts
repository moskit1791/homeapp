...
import { isRuntimeModuleEncrypted } from '../encryption-runtime';
...
export function updateHouseholdEncryptionSettings(
  ...
  return apiRequest<HouseholdEncryptionSettings, UpdateHouseholdEncryptionRequest>(
...
export function removeHouseholdEncryptionSettings(
  ...
  return apiRequest<HouseholdEncryptionSettings, RemoveHouseholdEncryptionRequest>(
...
export function exportHouseholdEncryptionData(
  ...
  return apiRequest<EncryptionExportItem[]>(`/encryption/household/export/${module}`, {
    ...requestOptions,
    method: 'GET',
  });
}
...
export function logoutSession(options?: ApiCallOptionsInput): Promise<OkResponse> {
  const requestOptions = normalizeApiCallOptions(options);
...
export function importExpenses(
  ...
  return apiRequest<ImportExpensesResult, ImportExpensesRequest>('/finance/expenses/import', {
    accessToken: requestOptions.accessToken,
    body: input,
    method: 'POST',
    signal: requestOptions.signal,
...
export function listDataEntries(
  ...
  const query = search && !encrypted ? `?search=${encodeURIComponent(search)}` : '';
  ...
  return apiRequest<DataEntry[]>(`/data-entries${query}`, {
    ...
    encrypted && normalizedSearch
      ? items.filter((item) =>
          `${item.title} ${item.value}`.toLocaleLowerCase('pl').includes(normalizedSearch)
        )
      : items
  );
}
...
export function listAttachments(
  ...
  return apiRequest<Attachment[]>(`/attachments${query}`, {
    ...
    encrypted && normalizedSearch
      ? items.filter((item) =>
          `${item.fileName} ${item.caption}`.toLocaleLowerCase('pl').includes(normalizedSearch)
        )
      : items
  );
}
...
export async function getPantryDashboard(options?: ApiCallOptionsInput): Promise<PantryDashboard> {
  const requestOptions = normalizeApiCallOptions(options);

  const dashboard = await apiRequest<PantryDashboard>('/shopping-lists/pantry/dashboard', {
    accessToken: requestOptions.accessToken,
    signal: requestOptions.signal,
  });
  ...
  return {
    ...
    stats: {
      ...dashboard.stats,
      expired: datedItems.filter((item) => new Date(`${item.expirationDate}T12:00:00`) < today)
        .length,
