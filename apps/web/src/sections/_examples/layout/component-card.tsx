...
import { COMPONENT_PACKAGE } from './nav-config-components';
...
export function ComponentCard({ item, sx, ...other }: ComponentCardProps) {
  ...
  return (
    <Box
      ...
      {...other}
    >
      {(item.packageType === COMPONENT_PACKAGE.CUSTOM ||
        item.packageType === COMPONENT_PACKAGE.THIRD_PARTY) && (
        <Label
          color={item.packageType === COMPONENT_PACKAGE.CUSTOM ? 'info' : 'default'}
          sx={{
            top: 8,
            right: 8,
            zIndex: 9,
