...
import { CalendarDays, CheckCircle2, RefreshCcw, Sparkles, Utensils, WalletCards } from '../../src/ui/icon';
...
export default function StartScreen() {
  ...
  const hasDashboardItems = Boolean(
    dashboard?.finance || upcomingEvents.length > 0 || mealEntries.length > 0 || todoPreview.length > 0
  );
        ...
        <>
          <View style={styles.hero}>
            ...
            <View style={styles.heroContent}>
              ...
              <Text style={styles.heroTitle}>Masz {getDashboardCount(upcomingEvents, mealEntries, todoPreview)} spraw do sprawdzenia.</Text>
              <View style={styles.pillRow}>
                <Text style={[styles.pill, styles.financePill]}>{formatMoney(dashboard.finance?.totalRemainingAmount)} zostaje</Text>
                <Text style={[styles.pill, styles.calendarPill]}>{upcomingEvents.length} wydarzeń</Text>
                <Text style={[styles.pill, styles.todoPill]}>{todoPreview.length} zadań</Text>
              </View>
            </View>
          </View>
          ...
          >
            {upcomingEvents.length > 0 ? (
              upcomingEvents.map((event) => (
                <ListRow
                  key={event.id}
                  meta={[formatDate(event.eventDate), event.eventTime].filter(Boolean).join(' / ')}
                  title={event.title}
                />
              ))
            ) : (
          ...
          <SectionCard icon={<Utensils color={theme.colors.food} size={17} />} title="Jedzenie">
          ...
          <SectionCard icon={<CheckCircle2 color={theme.colors.primary} size={17} />} title="To-do">
          ...
          {!hasDashboardItems ? <EmptyState text="Nie ma jeszcze danych do pokazania na starcie." /> : null}
        </>
      ) : null}
    </AppScreen>
  );
