...
type BudgetItemWithCategory = BudgetItem & { category: BudgetCategoryWithItems };
...
export default function FinanseScreen() {
  ...
  const flatItems = useMemo<BudgetItemWithCategory[]>(
    () => categories.flatMap((category) => getCategoryItems(category).map((item) => ({ ...item, category }))),
    [categories]
  );
  ...
  const budgetUsagePercent = budgetAmount > 0 ? Math.min(Math.max((spentAmount / budgetAmount) * 100, 0), 100) : 0;
  ...
  }, [categories, categoriesQuery.data, selectedItemCategoryId, setSelectedItemCategoryId]);
  ...
  const incomeMutation = useMutation({
    mutationFn: () =>
      upsertIncome(selectedIncomeMemberId, { amount: parseMoney(incomeAmount) }, { accessToken }),
    onSuccess: async () => {
  ...
  const canSaveIncome = canUpdate && Boolean(selectedIncomeMemberId) && isValidMoney(incomeAmount);
  ...
  const canSaveExpense = canCreate && Boolean(selectedExpenseItemId) && isPositiveMoney(expenseAmount);
  ...
  return (
    <AppScreen
      ...
      subtitle={currentMonth ? `Budżet domowy, ${formatMonth(currentMonth)}` : 'Brak bieżącego miesiąca'}
      title="Finanse"
    >
      <QueryState
        ...
        <>
          <View style={styles.hero}>
            <View style={styles.heroHeader}>
              ...
              <View style={styles.heroText}>
                ...
                <Text style={[styles.heroValue, remainingAmount < 0 && styles.heroValueDanger]}>
            ...
            <View style={styles.heroMetaRow}>
              <Text style={styles.heroMeta}>Wydane {formatMoney(spentAmount)}</Text>
              <Text style={styles.heroMeta}>Budżet {formatMoney(budgetAmount)}</Text>
            </View>
          </View>
          ...
          <View style={styles.summaryGrid}>
            ...
            <MetricCard
              ...
              label="Zostaje"
              style={[styles.metricCard, remainingAmount < 0 ? styles.metricDanger : styles.metricRemaining]}
          ...
          >
            ...
            {incomeMutation.error ? <InlineAlert tone="error" text="Nie udało się zapisać dochodu." /> : null}
          </SectionCard>
          ...
          >
            {canCreate ? (
              <View style={styles.formBox}>
                ...
                />
                {categoryMutation.error ? <InlineAlert tone="error" text="Nie udało się dodać kategorii." /> : null}
              </View>
            ) : null}

            {canCreate ? (
              <View style={styles.formBox}>
                <Text style={styles.boxTitle}>Nowa pozycja</Text>
                <TextField
                  control={control}
                  label="Nazwa pozycji"
                  name="itemName"
                ...
                <ActionButton
                  ...
                  title="Dodaj pozycję"
                />
                {itemMutation.error ? <InlineAlert tone="error" text="Nie udało się dodać pozycji." /> : null}
              </View>
            ) : null}

            <View style={styles.list}>
              {categories.length === 0 ? <InlineAlert text="Nie ma jeszcze kategorii budżetowych." /> : null}
              {categories.map((category) => (
                <CategoryBlock category={category} colors={theme.colors} key={category.id} />
              ))}
            </View>
          </SectionCard>
          ...
          >
            {canCreate ? (
              <View style={styles.formBox}>
                ...
                {expenseMutation.error ? <InlineAlert tone="error" text="Nie udało się dodać wydatku." /> : null}
              </View>
            ) : null}
            <View style={styles.list}>
              {flatItems.length === 0 ? <InlineAlert text="Dodaj pozycję, aby księgować wydatki." /> : null}
              {flatItems.map((item) => (
                <MoneyRow
                  key={item.id}
                  label={item.name}
          ...
          >
            ...
            <QueryState error={archiveQuery.error} isLoading={archiveQuery.isLoading} />
            <View style={styles.list}>
              {(archiveQuery.data ?? []).slice(0, 6).map((month) => (
                <MoneyRow
                  key={month.id}
                  label={formatMonth(month)}
                  sublabel={month.archivedAt ? `Zamknięty ${formatDate(month.archivedAt)}` : 'Archiwum'}
                  value=""
                />
              ))}
              {!archiveQuery.isLoading && (archiveQuery.data ?? []).length === 0 ? (
                <InlineAlert text="Archiwum jest jeszcze puste." />
              ) : null}
            </View>
            {nextMonthMutation.error ? (
              <InlineAlert tone="error" text="Nie udało się wygenerować kolejnego miesiąca." />
            ) : null}
          </SectionCard>
        </>
      ) : null}
...
function CategoryBlock({ category, colors }: { category: BudgetCategoryWithItems; colors: AppPalette }) {
    ...
    <View style={styles.categoryBlock}>
      <View style={styles.categoryHeader}>
        <View style={styles.categoryTitle}>
          ...
          <Text style={styles.muted}>
            {items.length} pozycji - {category.copyBudgetToNextMonth ? 'kopiowana' : 'jednorazowa'}
          </Text>
        </View>
        <Text style={[styles.categoryTotal, remaining < 0 && styles.moneyValueDanger]}>{formatMoney(remaining)}</Text>
      </View>
      <View style={styles.list}>
        {items.length === 0 ? <InlineAlert text="Brak pozycji w tej kategorii." /> : null}
        {items.map((item) => (
          <MoneyRow
            key={item.id}
            label={item.name}
            sublabel={`${formatOwner(item.owner)} - wydane ${formatMoney(item.spentAmount)}`}
            value={item.budgetAmount ? formatMoney(item.remainingAmount ?? 0) : 'Bez limitu'}
          />
        ))}
      </View>
    </View>
...
function MoneyRow({ label, sublabel, value }: { label: string; sublabel?: string; value: string }) {
  const theme = useAppTheme();
  const styles = useMemo(() => createStyles(theme.colors), [theme.colors]);
