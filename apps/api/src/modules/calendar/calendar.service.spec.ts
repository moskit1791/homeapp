...
describe('CalendarService', () => {
  it('sorts same-day all-day events when database timestamps are Date objects', async () => {
    ...
    const database = {
      query: vi
        .fn()
        .mockResolvedValueOnce({ rows: [{ current_time: '09:30', today: '2026-05-22' }] })
        .mockResolvedValueOnce({
          rows: [
            ...
            calendarEventRow({ created_at: createdAt, id: 'event-first', title: 'Spisac gaz' })
          ]
        })
    };
  ...
  it('does not return timed events from earlier today as upcoming', async () => {
    const database = {
      query: vi
        .fn()
        .mockResolvedValueOnce({ rows: [{ current_time: '23:30', today: '2026-05-25' }] })
        .mockResolvedValueOnce({
          rows: [
            calendarEventRow({ event_date: '2026-05-25', event_time: '10:00', id: 'past-today' }),
            calendarEventRow({ event_date: '2026-05-25', event_time: '23:45', id: 'later-today' }),
            calendarEventRow({ event_date: '2026-05-26', event_time: '08:00', id: 'tomorrow' })
          ]
        })
    };
    const service = new CalendarService(
    ...
    expect(events.map((event) => event.id)).toEqual(['later-today', 'tomorrow']);
  });
});
