...
export const SHOPPING_CATEGORY_META: Record<ShoppingCategory, ShoppingCategoryMeta> = {
  ...
  'Mięso i wędliny': { color: '#dc2626', emoji: '🥩', title: 'Mięso i wędliny' },
  ...
  'Pieczenie i dodatki': { color: '#fb7185', emoji: '🧁', title: 'Pieczenie i dodatki' },
  ...
  'Środki czystości': { color: '#14b8a6', emoji: '🧽', title: 'Środki czystości' },
...
export const SHOPPING_PRODUCT_CATALOG = [
  ...
  shoppingProduct('czarne porzeczki', 'Owoce, warzywa i zioła', ['czarna porzeczka']),
...
const SHOPPING_CATEGORY_BY_NORMALIZED = new Map(
  SHOPPING_CATEGORIES.map((category) => [normalizeShoppingText(category), category] as const)
);
...
const SHOPPING_PRODUCT_CATEGORY_BY_NAME = new Map<string, ShoppingCategory>(
  SHOPPING_PRODUCT_CATALOG.flatMap((item) => {
    ...
    return terms.map((term) => [normalizeShoppingText(term), item.category] as const);
  })
);
...
const SHOPPING_CATEGORY_KEYWORD_RULES: readonly ShoppingKeywordRule[] = [
  { category: 'Alkohole', pattern: /\b(piwo|wino|alkohol|wodka|whisky|prosecco)\b/ },
  { category: 'Apteczka', pattern: /\b(witamin|lek|tablet|syrop|aptecz|plastry|termometr)\b/ },
  { category: 'Dania gotowe', pattern: /\b(pierog|hamburger|pizza|barszcz|zupa|gotow)\b/ },
  { category: 'Dla zwierząt', pattern: /\b(karma|zwierzat|psa|kota|kuwet)\b/ },
  { category: 'Dom i ogród', pattern: /\b(folia|ogrod|grill|worki|znicz|donicz)\b/ },
  { category: 'Dziecko', pattern: /\b(pieluch|pampers|dzieck|niemowl|chusteczki mokre)\b/ },
  { category: 'Elektronika', pattern: /\b(bateri|ladowark|kabel|zarowk|usb)\b/ },
  {
    category: 'Higiena',
    pattern:
  ...
  { category: 'Papiernicze', pattern: /\b(zeszyt|dlugopis|olowek|papiernicz|blok|notes)\b/ },
  {
    ...
    pattern: /\b(kakao|proszek do pieczenia|maka tortowa|soda|budyn|galaretk|wiork|bulka tarta|papier do pieczenia)\b/
  },
  { category: 'Pieczywo', pattern: /\b(chleb|bulka|bulki|bagiet|kajzer|tost|pieczyw|croissant|drozdz)\b/ },
  {
  ...
  { category: 'Ryby i owoce morza', pattern: /\b(ryb|losos|tunczyk|dorsz|sledz|krab|krewet)\b/ },
  {
  ...
  { category: 'Ubrania', pattern: /\b(ubran|koszul|spodn|skarpet|buty|czapk)\b/ },
  { category: 'Wege', pattern: /\b(ciecierzyc|soczewic|tofu|wege|fasola)\b/ },
  { category: 'Woda i napoje', pattern: /\b(woda|sok|napoj|cola|pepsi|energetyk|smoothie|mineralna)\b/ }
];
...
export function isShoppingCategory(value: string | null | undefined): value is ShoppingCategory {
  return Boolean(value && SHOPPING_CATEGORY_BY_NORMALIZED.has(normalizeShoppingText(value)));
}

export function getShoppingCategoryMeta(value: string | null | undefined): ShoppingCategoryMeta {
  const category =
...
export function categorizeShoppingProduct(value: string): ShoppingCategory {
  ...
  return SHOPPING_CATEGORY_KEYWORD_RULES.find((rule) => rule.pattern.test(normalized))?.category ?? 'Inne';
}
...
export function getShoppingProductSuggestions(
  ...
  for (const entry of SHOPPING_PRODUCT_SEARCH_INDEX) {
    ...
    const contains = startsWith || entry.terms.some((term) => term.includes(normalized));
